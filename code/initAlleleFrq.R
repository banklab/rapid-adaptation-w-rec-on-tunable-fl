...
pdf(paste(args[1], add, "intFixAlleleFrq.pdf", sep="."), width=2, height=2)
...
for(L in c(5, 15)){
    for(a in add){
       ...
       #     if(k==3){
        #        legend(1, 0.5, legend=popSize, col=rgb(red=red, green=green, blue=blue),  bty="n", pch=rep(15, 3), title="PopSize", cex=0.8)
         #       legend(3, 0.5, legend=c(rep(c(0.001, 0.01, 0.1, 0.5), 1)), col=rgb(red=0, green = 0, blue=0, alpha=alpha[c(2,3,4, 5)]), seg.len=0.7, lwd=1.5,lty = rep(1,4), bty="n", title = "Recomb.Prob.", cex=0.8)
          #  }
            ...
            #rmf_L5_N5000_mua0.01_sa0_sb0.1.fixAlleleInitFreq
            ...
            for(N in popSize){
                ...
                tmp=matrix(, ncol=5, nrow=0)
                for(i in seq(6, l, 3)){
                    tmp=rbind(tmp,tapply(data[,i], data[,2], mean))
                }
                
                t=1
                for(c in  1:5){
                    lines(1:L, tmp[,c], col=rgb(red[j], green[j], blue[j], alpha=alpha[t]), lty=ltype[t])
                    t=t+1
                }
                j=j+1
            }
