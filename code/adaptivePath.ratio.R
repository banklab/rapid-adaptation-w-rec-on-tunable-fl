...
pdf(paste(args[1], add, "adaptivePath.ratio.pdf", sep="."), width=6, height=2)
...
for(L in c(5, 15)){
    for(a in add){
        for(e in epi){
            ...
            if(e>=a){
                axis(2, at=values, labels = c("0.0", 0.5, "1.0",1.5, "2.0"), cex.axis=0.8)
            }
            else{
                print(seq(ylow, yup, stepSize))
                axis(2, at=seq(ylow, yup, stepSize), labels = c("0.00", 0.05, "0.10", "0.15", "0.20"), cex.axis=0.8)
            ...
            for(N in popSize){
                ...
                #finalMeanAbsRatio=matrix(, ncol=5, nrow=0)
                #majorMeanAbsRatio=matrix(, ncol=5, nrow=0)
                ...
                t=1
                for(c in  1:5){
                    lines(1:(L-1)+0.5, finalMeanRatio[,c], col=rgb(red[j], green[j], blue[j], alpha=alpha[t]), lty=ltype[t])
                    points(1:(L-1)+0.5, finalMeanRatio[,c], col=rgb(red[j], green[j], blue[j], alpha=alpha[t]), pch=3, cex=0.4)
                    
                    lines(1:(L-1)+0.5, majorMeanRatio[,c], col=rgb(red[j], green[j], blue[j], alpha=alpha[t]), lty=ltype[t])
                    points(1:(L-1)+0.5, majorMeanRatio[,c], col=rgb(red[j], green[j], blue[j], alpha=alpha[t]), pch=4, cex=0.4)
                    t=t+1
