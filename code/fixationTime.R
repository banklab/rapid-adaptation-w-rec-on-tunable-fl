...
pdf(paste(args[1], add, "fixationTime.pdf", sep="."), width=2, height=2)
...
for(a in add){
    for(e in epi){
        boxplot(NULL, xlim=c(0,10), xlab="", ylab="", ylim=c(-2, 0.7), yaxt="n")
        ...
        #if(k==1 || k==4 || k==7){
            mtext("Fixation time (N)", side=2, line=2, cex=0.8, las=0)
            #}
        ...
        #if(k==1){
        #    legend(0,log(0.4), legend=c("100", "500", "5000"), title="PopSize", col=rgb(red=red, green=green, blue=blue),  bty="n", pch=c(15,15,15), cex=0.8)
        #    legend(3, log(0.3), legend=c("L=5", "L=15"), pch=dots, bty="n", cex=0.8)
        #}
        k=k+1   
        ...
        for(N in popSize){
            ...
            for(L in c(5, 15)){
                ...
                lines(seq(i,i+8,2), tapply(log10(data$tfixation), data$recombination_rate, quantile, p=0.5), col=rgb(red[j], green[j], blue[j]), lwd=1)
                points(seq(i,i+8,2), tapply(log10(data$tfixation), data$recombination_rate, quantile, p=0.5), col=rgb(red[j], green[j], blue[j]), pch=dots[c1],  cex=0.8)
