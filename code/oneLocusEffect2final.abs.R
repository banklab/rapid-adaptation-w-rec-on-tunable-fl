...
pdf(paste(args[1], add, "oneLocEffect2final.abs.pdf", sep="."), width=6, height=2)
...
for(L in c(5, 15)){
    for(a in add){
        for(e in epi){
            ...
            if(k==1 || k==4){
            mtext("One-locus effect(abs.)", side=2, line=2.6, las=0, cex=0.8)
            }
            ...
            for(N in popSize){
                ...
                for(i in 1:L){
                    tmpMean=rbind(tmpMean,tapply(abs(data[,i+3]), data[,2], mean))
                    tmpMeanM=rbind(tmpMeanM,tapply(abs(major[,i+3]), major[,2], mean))
